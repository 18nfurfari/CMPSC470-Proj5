...
fun_decl        : FUNC IDENT LPAREN params RPAREN FUNCRET prim_type BEGIN local_decls{ Debug("fun_decl -> FUNC ID(params)->prim_type BEGIN local_decls"); $<obj>$ = fundecl____FUNC_IDENT_LPAREN_params_RPAREN_FUNCRET_primtype_BEGIN_localdecls_10X_stmtlist_END($2, $4, $7, $9); }
                                                                        stmt_list END{ Debug("                                           stmt_list END"); $$ =      fundecl____FUNC_IDENT_LPAREN_params_RPAREN_FUNCRET_primtype_BEGIN_localdecls_X10_stmtlist_END($2, $4, $7, $9, $10, $11); }
                ;

params          : param_list                                               { Debug("params -> param_list"                         ); $$ = params____paramlist($1); }
                |                                               { Debug("param -> eps"                         ); $$ = params____eps(); }
                ;
...
param     : VAR  type_spec  IDENT                       { Debug("param -> VAR  type_spec  IDENT"); $$ = param____VAR_typespec_IDENT($2, $3); }
...
if_stmt     : IF  LPAREN  expr  RPAREN  stmt  ELSE  stmt        { Debug("if_stmt -> IF  LPAREN  expr  RPAREN  stmt  ELSE  stmt"          ); $$ = ifstmt____IF_LPAREN_expr_RPAREN_stmt_ELSE_stmt($3, $5, $7); }
                ;

while_stmt     : WHILE  LPAREN  expr  RPAREN  stmt       { Debug("while_stmt -> WHILE  LPAREN  expr  RPAREN  stmt") ; $$ = whilestmt____WHILE_LPAREN_expr_RPAREN_stmt($3, $5); }
               ;
...
arg_list        : arg_list  COMMA  expr                         { Debug("arg_list -> arg_list COMMA expr"                           ); $$ = arglist____arglist_COMMA_expr($1, $3); }
                | expr                                          { Debug("expr"); $$ = arglist____expr($1);}
                ;
